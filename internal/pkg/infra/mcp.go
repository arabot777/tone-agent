...
func InitMCP(conf *conf.BaseConfig) {
	var err error
	MCPServer, err = CreateMCPClients(conf)
	if err != nil {
		panic(err)
	}
}
...
func CreateMCPClients(conf *conf.BaseConfig) (map[string]client.MCPClient, error) {
	// 将 DeerConfig 转换为 MCPConfig
	mcpConfig := &MCPConfig{
		MCPServers: make(map[string]ServerConfigWrapper),
	}

	for name, server := range conf.MCP.Servers {
	...
	for name, server := range mcpConfig.MCPServers {
		...
		if server.Config.GetType() == transportSSE {
			...
			// 使用新版本API：NewSSEMCPClient返回(*Client, error)，无需调用Start()
